...
export class NovoAnimalComponent implements OnInit {
  ...
  upload() {
    const allowComments = this.formularioAnimal.get('allowCooments')?.value ?? false;
    ...
    .pipe(
      finalize(()=> this.router.navigate(['animais']))
        ...
        if(event.type == HttpEventType.UploadProgress) {
          ...
          this.porcentualConcludo = Math.round(100*(event.loaded / total));
        }
      }, (error)=> console.error(error)
      );
  }

  gravaArquivo(aruqivo: any): void {
    const [file] = aruqivo?.files;
    this.file = file;
